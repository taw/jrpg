...
class WorldOutside:
    def __init__(self, wm, ui, mhc):
        ...
        def open_castle_gate():
            ...
            ui.change_text(["Oh, you're a hero. Please come in."])
        ...
        def castle_gate_checkup():
            ...
            if mhc.quest_is_done("desert artifact") and mhc.quest_is_done("magic sword complete"):
                ...
                ui.change_text([
                    ...
                    "Come back once you get a magic sword and a spellbook",
                ])
        ...
        def elven_gate_soldier_event():
            if gate_was_open:
                ui.change_text(["Welcome to the Elven town."])
            else:
                ...
                if mhc.has_money(2):
                    ...
                    open_elven_gate()
                    ui.change_text(["OK, you may come in now."])
                else:
                    ui.change_text([
                        "You must pay a one-time 2 coins tax to pass.",
        ...
        def elven_bridge_check_xp():
            if mhc.quest_is_done("elven bridge ok"):
                return
            ui.change_text([
                    "The inexperienced should not cross the bridge, it's too dangerous.",
        ...
        def bridge_soldier_event():
            if bridge_was_ok:
                ui.change_text(["If the demons on the other side are too hard,",
        ...
        def arab_trader_quest():
            if mhc.quest_is_done("antiheat potion complete"):
                ui.change_text(["So you've drunk the potion.", "You may go to the desert now."])
            else:
                ui.change_text([
        ...
        wm.add_chara("elf-trader", route=[(3, 23), (3, 24)], event=lambda: ui.change_text([
                "Many animals and people have been possessed by kanji demons.",
        ...
        ###############################
        def meadow_quest():
            ui.change_text(["You've found the magic sword but it's broken."])
        ...
        def get_mushroom_bgreen(item):
            if not mhc.quest_is_done("antiheat potion bgreen"):
                ...
                ui.change_text([
                    "You've got bright green mushrooms.",
                    "You need yellow and bright green mushrooms for the potion."])
        ...
        def desert_quest():
            ui.change_text(["You've got a spellbook."])
        ...
        def desert_heat():
            if not mhc.quest_is_done("antiheat potion complete"):
                if random() < 0.25:
                    ui.change_text(["The heat is too strong.", "You lost 1 HP."])
                    mhc.damage(1)
                else:
                    ui.change_text(["The heat is too strong.", "Maybe you should return already."])
        if not mhc.quest_is_done("antiheat potion complete"):
...
class WorldIcyMountains:
    def __init__(self, wm, ui, mhc):
        ...
        def ice_quest():
            ui.change_text(["You've got a red spellbook."])
